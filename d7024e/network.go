...
func (network *Network) Listen() {
	...
	if err != nil {
		log.Println("HandleFindNode: ResolveUDPAddr error, failed to resolve address: " + GetIPAddress())
		log.Fatal(err)
	}
...
func (network *Network) HandleRequest(recPacket *Packet, conn *net.UDPConn, rAddr *net.UDPAddr) {
	log.Println("Received " + recPacket.Type + " packet from ID: " + recPacket.KademliaID.String() + ", IP: " + recPacket.Address)
...
func (network *Network) SendResponse(packetType string, data []byte, conn *net.UDPConn, rAddr *net.UDPAddr) {
	...
	log.Println("Sending " + packetType + " packet to " + rAddr.IP.String())
	_, err := conn.WriteToUDP(b, rAddr)
	if err != nil {
		log.Println("Error writing FOUND_K_NODES packet")
		log.Fatal(err.Error())
	}
}
...
func (network *Network) HandleFindNode(recPacket *Packet, conn *net.UDPConn, rAddr *net.UDPAddr) {
	...
	if err != nil {
		log.Println("error marshaling closestContacts")
		log.Fatal(err)
	}
...
func (network *Network) HandleStore(recPacket *Packet, conn *net.UDPConn, rAddr *net.UDPAddr) {
	...
	log.Println("Saved string: '" + string(data.Value) + "' in storage")
...
func (network *Network) HandleFindData(recPacket *Packet, conn *net.UDPConn, rAddr *net.UDPAddr) {
	...
	if data != nil {
		...
		if err != nil {
			log.Println("HandleStore: Error marshaling data")
		}
		...
		byte, err := json.Marshal(closestContacts)
		if err != nil {
			log.Println("HandleFindData: error marshaling closestContacts")
			log.Fatal(err)
		}
...
func (network *Network) SendMessage() {
	...
	for {
		...
		go func(message *Message) {
			...
			if err != nil {
				log.Println("SendMessage: ResolveUDPAddr error, failed to resolve remote address: " + message.IPAddress)
			}

			conn, err := net.DialUDP("udp", nil, udpAddr)
			if err != nil {
				log.Println("SendMessage: DialUDP error, failed dialing " + udpAddr.IP.String() + ":" + strconv.Itoa(udpAddr.Port))
			}
			conn.SetDeadline(time.Now().Add(100 * time.Millisecond))
			defer conn.Close()
			// Send encoded packet to contact

			log.Println("Sending " + message.Packet.Type + " packet to " + message.IPAddress)
			_, err = conn.Write(b)
			if err != nil {
				log.Println("Error writing " + message.Packet.Type + " packet")
				log.Println(err)
			}
			...
			if err != nil {
				log.Println("TIMEOUT!")
				log.Println(message.Packet.Type + " message timed out!")
				recPacket.Type = "TIMEOUT"
			} else {
				json.Unmarshal(buffer[:size], &recPacket)
				log.Println("Received " + recPacket.Type + " response from " + message.IPAddress)
			}
...
func (network *Network) SendFindDataMessage(node *Node, key *KademliaID) *Packet {
	...
	if err != nil {
		log.Println("SendFindDataMessage: Error marshaling data")
	}
...
func (network *Network) SendStoreMessage(contact *Contact, data *Data) {
	dataByte, err := json.Marshal(data)
	if err != nil {
		log.Println("SendStoreMessage: Error marshaling data")
	}
